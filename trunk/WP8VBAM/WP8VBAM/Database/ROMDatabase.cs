    ...
    class ROMDatabase : IDisposable
    {
        ...
        public ROMDBEntry GetROMFromSavestateName(string savestateName)
        {
            ...
            return this.context.ROMTable
                ...
                .FirstOrDefault();
            //return this.context.ROMTable
            //    .Where(r => r.FileName.ToLower().StartsWith(name))
            //    .FirstOrDefault(); 
            //return this.context.ROMTable
            //     .Where(r => r.FileName.Substring(0, r.FileName.Length - (r.FileName.Length - r.FileName.LastIndexOf('.'))).ToLower().Equals(name))
            //     .FirstOrDefault();
        }
        ...
        public int GetLastSavestateSlotByFileNameExceptAuto(string filename)
        {
            ...
            SavestateEntry save = this.context.SavestateTable
                .Where(s => s.ROMFileName.ToLower().Equals(filename) && s.Slot != 9)
